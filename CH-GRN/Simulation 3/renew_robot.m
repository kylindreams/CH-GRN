...
%                 g3 = robot(id).generate_pattern(target,robot,pattern_barrier); %先生成每个机器人的pattern
%                 robot(id).pattern = g3;
                ...
                [move,die] = robot(id).trapping(newrobot,target,pattern_barrier,robot(id).pattern);
                move = newrobot(id).legal_action(move);
        ...
        %%如果浓度很大且变化很小，不改变速度
        xaxis = fix(newrobot(id).nowx/0.25)+1;   
        yaxis = fix(newrobot(id).nowy/0.25)+1;
