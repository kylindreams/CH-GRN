...
for id = 1:num_robot
    robot(id) = Robot(id,rand()*2+9,rand()*2+10.5,rand(),rand(),1);
end
...
global obstacle
obstacle = [7,6,2,10;11,6,2,10];%窄道
...
for i = 1:N
    change = 0;    
    %障碍物位置更新
    [pattern_barrier,simulation_robot,pattern_simulationrobot] = renew_barrier(barrier,change);    
    %pattern形成
